    ...
    , ui(new Ui::Widget)
{
    ...
    this->setMaximumSize(372,521);
    this->setMaximumSize(372,521);

    this->setWindowTitle("计数器");
...
//    ui->mainLineEdit->setText(expression);
    ...
    strcpy(opt,ba.data());  //QByteArray -> const char *

    //用QScriptEngine可以直接算出数据
    ...
    expression.clear(); //每计算一次后清零

    /////QStringtext=ui->resultLineEdit->text();
    /// if(text==NULL||isNumber(text)){
    /// ui->resultLineEdit->setText(QString::number(s_num.top()));}
